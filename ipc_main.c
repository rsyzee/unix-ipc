...
static struct unix_ipc_ctx *init_ipc_srv_ctx(const struct ipc_config* ipc_proto_config) {
	...
	// Performance: increase socket buffer sizes
	int sndbuf = 65536;
	int rcvbuf = 65536;
...
int ipc_send_msg(struct unix_ipc_ctx *ipc_handle, struct ipc_message_pool* msg) {
	...
	iov[0].iov_len  = sizeof(struct ipc_message_header);

	iov[1].iov_base = msg->payload;
	iov[1].iov_len  = msg->header.payload_len;

	msg_hdr.msg_iov = iov;
	msg_hdr.msg_iovlen = 2;

	ssize_t total = sizeof(struct ipc_message_header) + msg->header.payload_len;
	ssize_t sent = sendmsg(ipc_handle->stream_fd, &msg_hdr, MSG_NOSIGNAL);

	if (sent != total) {
		HANDLE_ERROR(ipc_handle, "Failed to send message", errno);
		return -1;
	}
...
int ipc_recv_msg(const struct unix_ipc_ctx *ipc_handle, struct ipc_message_pool* msg) {
	...
	if (received != sizeof(struct ipc_message_header)) {
		HANDLE_ERROR(ipc_handle, "Incomplete header received", errno);
		return -1;
	}
	...
	if (received != (ssize_t)msg->header.payload_len) {
...
static void subs_to_client_event(struct unix_ipc_ctx *ctx, void *storage_msg_pool)
{
	...
	while (!g_need_exit)
	{
		...
		build_proto_msg(&msg_to_send, temp_buffer, len + 1);
